...
| Track an update | "I applied to X", "I got rejected", "I have an interview at X" | `/track` |
...
- `postings/tracker.yaml` — Application lifecycle tracker
...
10. **Keep the application tracker current.** Update `postings/tracker.yaml` on every status change. Valid statuses: `saved`, `applying`, `applied`, `interviewing`, `offered`, `accepted`, `rejected`, `withdrawn`, `closed`. See `templates/tracker.template.yaml` for schema.
